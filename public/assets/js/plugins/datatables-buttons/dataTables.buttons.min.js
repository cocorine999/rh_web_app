...
(function (e) {
    ...
    e.extend(v.prototype, {
        ...
        add: function (a, b) {
            ...
            this._expandButton(
                ...
                a,
                a !== q ? a.split : q,
                (a === q || a.split === q || 0 === a.split.length) && d !== q,
                !1,
                b
            );
            this._draw();
        ...
        collectionRebuild: function (a, b) {
            ...
            for (c = 0; c < b.length; c++)
                this._expandButton(a.buttons, b[c], !0, c);
        ...
        _constructor: function () {
            ...
            e("body").on("keyup." + this.s.namespace, function (l) {
                if (!y.activeElement || y.activeElement === y.body) {
                    ...
                    -1 !== a.s.listenKeys.toLowerCase().indexOf(k) &&
                        a._keypress(k, l);
                }
            });
        },
        ...
        _expandButton: function (a, b, c, d, h, g, l) {
            ...
            for (var r = p.length; c < r; c++) {
                ...
                if (f)
                    if (
                        ((b = f.config !== q && f.config.split ? !0 : !1),
                        Array.isArray(f))
                    )
                        this._expandButton(
                            a,
                            f,
                            m !== q && m.conf !== q ? m.conf.split : q,
                            d,
                            l !== q && l.split !== q,
                            g,
                            l
                        ...
                        var m = this._buildButton(
                            f,
                            d,
                            f.split !== q ||
                                (f.config !== q && f.config.split !== q),
                            h
                        );
                        if (m) {
                            g !== q && null !== g
                            ...
                            if (m.conf.buttons || m.conf.split) {
                                ...
                                if (m.conf.split)
                                    for (
                                        var u = 0;
                                        u < m.conf.split.length;
                                        u++
                                    )
                                        "object" === typeof m.conf.split[u] &&
                                            ((m.conf.split[c].parent = l),
                                            m.conf.split[u].collectionLayout ===
                                                q &&
                                                (m.conf.split[
                                                    u
                                                ].collectionLayout =
                                                    m.conf.collectionLayout),
                                            m.conf.split[u].dropup === q &&
                                                (m.conf.split[u].dropup =
                                                    m.conf.dropup),
                                            m.conf.split[u].fade === q &&
                                                (m.conf.split[u].fade =
                                                    m.conf.fade));
                                else
                                    e(m.node).append(
                            ...
                            m.conf.parent = l;
                            f.init &&
                                f.init.call(k.button(m.node), k, e(m.node), f);
                            n++;
                        }
                    }
            }
        ...
        _buildButton: function (a, b, c, d) {
            ...
            !c && d && k.buttonLiner
                ? (g = k.buttonLiner)
                : !c && b && l.buttonLiner && (g = l.buttonLiner);
            if (a.available && !a.available(p, a) && !a.hasOwnProperty("html"))
                return !1;
            ...
            if (c) {
                ...
                var G = function (w, A, C, F) {
                        z.split.action.call(
                            A.button(e("div.dt-btn-split-wrapper")[0]),
                            w,
                            A,
                            C,
                            F
                        );
                        e(A.table().node()).triggerHandler(
                            "buttons-action.dt",
                            [A.button(C), A, C, F]
                        );
        ...
        _resolveExtends: function (a) {
            var b = this.s.dt,
                ...
                d = function (k) {
                    for (
                        var n = 0;
                        !e.isPlainObject(k) && !Array.isArray(k);
            ...
            for (a = d(a); a && a.extend; ) {
                ...
                a.config !== q &&
                    h.config !== q &&
                    (a.config = e.extend({}, h.config, a.config));
                a = e.extend({}, h, a);
        ...
        _popover: function (a, b, c) {
            ...
            "dt-container" === h.align &&
                ((g = g.parent()), c.css("width", k.width()));
            if ("absolute" === d) {
                var n = g.position();
                d = e(b.node()).position();
                c.css({
                ...
                if (
                    ...
                    g.hasClass("dt-btn-split-wrapper") &&
                        ...
                        (u = m + f));
                    f = 0;
                    if (c.hasClass(h.rightAlignClassName))
                        (f = r - p),
                            d > n + f &&
                                ((d -= n + f),
                                (k -= p + f),
                                (f = d > k ? f + k : f + d));
                    else if (c.hasClass(h.splitRightAlignClassName))
                        (f = u - p),
                            d > n + f &&
                                ((d -= n + f),
                                (k -= p + f),
                    ...
                    (d = g.offset().top),
                        (f = 0),
                        (f = "button-right" === h.align ? r - p : m - n);
                c.css("left", c.position().left + f);
            } else
                (d = c.height() / 2),
                    d > e(B).height() / 2 && (d = e(B).height() / 2),
                    c.css("marginTop", -1 * d);
            ...
            e("body")
                .on("click.dtb-collection", function (x) {
                    ...
                    ((!e(x.target).parents()[D]().filter(a).length &&
                        !e(G).hasClass("dt-buttons")) ||
                        e(x.target).hasClass("dt-button-background")) &&
                        l();
                })
    ...
    v.instanceSelector = function (a, b) {
        ...
        var c = [],
            ...
            h = function (g) {
                if (Array.isArray(g))
                    for (var l = 0, k = g.length; l < k; l++) h(g[l]);
                else
                    "string" === typeof g
                        ? -1 !== g.indexOf(",")
                            ? h(g.split(","))
                            : ((g = e.inArray(g.trim(), d)),
                              -1 !== g && c.push(b[g].inst))
                        : "number" === typeof g && c.push(b[g].inst);
            };
        h(a);
        return c;
    };
    v.buttonSelector = function (a, b) {
        for (
            var c = [],
                d = function (k, n, p) {
                    for (var r, f, m = 0, u = n.length; m < u; m++)
                        if ((r = n[m]))
                            (f = p !== q ? p + m : m + ""),
                                k.push({
                                    node: r.node,
                ...
                h = function (k, n) {
                    ...
                    if (Array.isArray(k) || k instanceof e)
                        for (f = 0, p = k.length; f < p; f++) h(k[f], n);
                    else if (null === k || k === q || "*" === k)
                        for (f = 0, p = r.length; f < p; f++)
                            c.push({ inst: n, node: r[f].node });
                    else if ("number" === typeof k)
                        c.push({ inst: n, node: n.s.buttons[k].node });
                    else if ("string" === typeof k)
                        if (-1 !== k.indexOf(","))
                            for (
                                r = k.split(","), f = 0, p = r.length;
                                f < p;
                                f++
                            )
                                h(r[f].trim(), n);
                        ...
                        else if (-1 !== k.indexOf(":name"))
                            for (
                                k = k.replace(":name", ""), f = 0, p = r.length;
                                f < p;
                                f++
                            )
                                r[f].name === k &&
    ...
    var K = function (a) {
        ...
        Q = function (a, b) {
            ...
            a.select &&
                ...
                h.selected === q &&
                a.rows(c.rows, e.extend({ selected: !0 }, h)).any() &&
                e.extend(h, { selected: !0 });
            ...
            for (
                ...
                for (var f = [l], m = 0; m < l; m++)
                    (f[m] = c.format.body(h[n], p, m, g[n])), n++;
                k[p] = f;
            }
